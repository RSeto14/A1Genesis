...
class A1CPGEnv:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False, device="cuda",capture=False, eval=False):
        ...
        self.scene = gs.Scene(
            ...
            vis_options=gs.options.VisOptions(n_rendered_envs=num_envs, plane_reflection=False,shadow=True),
        ...
        # self.scene.add_entity(gs.morphs.Terrain(n_subterrains=(2,2),subterrain_types=[["flat_terrain","random_uniform_terrain"],["discrete_obstacles_terrain","stairs_terrain"]],visualization=True))
        # self.scene.add_entity(gs.morphs.Terrain(n_subterrains=(1,1),subterrain_types=[["fractal_terrain"]],visualization=True))
        ...
        # self.put_box(n_row=20,n_col=20,box_size=0.3,box_height_range=[0.01,0.12])
        ...
        # self.last_target_dof_pos = self.dof_pos

        
        # print(self.terrain.geoms[0].friction)
    ...
    def CPG(self, actions):
        # self.mu = (self.env_cfg["action_scale_mu"][1] - self.env_cfg["action_scale_mu"][0])/2 * actions[:, :4] + (self.env_cfg["action_scale_mu"][0]+self.env_cfg["action_scale_mu"][1])/2
        # self.omega = (self.env_cfg["action_scale_omega"][1] - self.env_cfg["action_scale_omega"][0])/2 * actions[:, 4:8] + (self.env_cfg["action_scale_omega"][0]+self.env_cfg["action_scale_omega"][1])/2
        # self.psi = (self.env_cfg["action_scale_psi"][1] - self.env_cfg["action_scale_psi"][0])/2 * actions[:, 8:12] + (self.env_cfg["action_scale_psi"][0]+self.env_cfg["action_scale_psi"][1])/2
        ...
        self.mu = torch.clip(self.mu, self.env_cfg["action_scale_mu"][0], self.env_cfg["action_scale_mu"][1])
        self.omega = torch.clip(self.omega, self.env_cfg["action_scale_omega"][0], self.env_cfg["action_scale_omega"][1])
        self.psi = torch.clip(self.psi, self.env_cfg["action_scale_psi"][0], self.env_cfg["action_scale_psi"][1])
    ...
    def _reward_body_height(self):
        return torch.abs(self.base_pos[:, 2] - self.env_cfg["h"][0])
...
if __name__ == "__main__":
    def get_cfgs():
        env_cfg = {
            ...
            "base_init_pos": [2.0, 2.0, 0.35],
            "base_init_quat": [1.0, 0.0, 0.0, 0.0],
            "episode_length_s": 20.0,
            "resampling_time_s": 4.0,
            "gain_resampling_time_s": 2.0,
            "friction_resampling_time_s": 2.0,
            "action_scale_mu": [1.0, 1.0],
            "action_scale_omega": [0.0, 0.0],
            # "action_scale_omega": [-1.5, 1.5],
            "action_scale_psi": [0.0, 0.0],
    ...
    env = A1CPGEnv(num_envs=1, env_cfg=env_cfg, obs_cfg=obs_cfg, reward_cfg=reward_cfg, command_cfg=command_cfg, device="cuda:0",capture=True, eval=True)

    obs, _ = env.reset()
    env.r = torch.ones(4,device="cuda:0")
    with torch.no_grad():
        # while True:
        if env.capture:
            env.cam.start_recording()
        for i in range(10):
            actions = torch.zeros(1,12,device="cuda:0")
            actions[0,0:4] = -100
            actions[0,4:8] = -100
